  ...
  const NwtFormulatorFeatureMixer = class {
    ...
    static async extractFeaturesInheritance(features, loaded = []) {
      ...
      for(let index=0; index<features.length; index++) {
        ...
        assertion(typeof feature.abstraction === "object", `Parameter «features[${index}].abstraction» must be object on «NwtFormulatorFeatureMixer.extractFeaturesInheritance»`);
        assertion(typeof feature.abstraction.name === "string", `Parameter «features[${index}].abstraction.name» must be string on «NwtFormulatorFeatureMixer.extractFeaturesInheritance»`);
        if(feature.inherits) {
    ...
    static async mix(features) {
      ...
      for (const interfaze of interfazes) {
        ...
        Plugin_for_class_specification: {
          if (interfaze.abstraction) {
            if(typeof out.abstraction.name !== "undefined") {
              if(!out.abstraction.seeds) {
                out.abstraction.seeds = [];
              }
              out.abstraction.seeds.push(out.abstraction.name);
              className = out.abstraction.name;
            }
            for (let prop in interfaze.abstraction) {
              const val = interfaze.abstraction[prop];
              if (prop === "definition") {
                if (typeof out.abstraction.definition !== "object") {
                  out.abstraction.definition = {};
                }
                Object.assign(out.abstraction.definition, val);
              } else {
                // The others override by themselves:
                Object.assign(out.abstraction, { [prop]: val });
              }
            }
          }
        }
        ...
        Plugin_for_settings_specification: {
          if (interfaze.abstraction.settings) {
            this.assignWithoutOverriding(out.abstraction.settings, interfaze.abstraction.settings);
          }
          if(interfaze.abstraction.settingsOverrider) {
            Object.assign(out.abstraction.settings, interfaze.abstraction.settingsOverrider);
          }
        }
        // data
        Push_data_callback:
        ...
        for (const collection of ["methods", "computed", "props"]) {
          ...
          for (const name in interfaze[collection]) {
            ...
            if (out[collection][name]) {
              throw new Error(`Collision on «${collection}.${name}» on class «${className}» at class sequence «${out.abstraction.seeds.join(",")}» on «NwtFormulatorFeatureMixer.mix»`);
            }
